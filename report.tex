...
{\bf LEO: Replaced all $...$ by $\dots$}

\section{Generalization of the construction for integers}
In this section we present first generalization of the decoding algorithm presented in \cite{[DP19]}. In their paper Léo Ducas and Cécile Pierrot use properties of the group $(\ZZ/m\ZZ)^*$ for modulus $m$ which is a prime power. The decoding radius achievable in polynomial-time depends on the ratio
\[
\frac{m}{\phi(m)}
\]
The higher the ratio the \toimprove{biggier} the \added{achievable decoding} radius is. In our generalization we take $m'$ an \added{arbitrary} product of prime powers. If $m$ and $m'$ are of the same size, as $m'$ is smoother $\phi(m')$ will be smaller which gives us a better decoding radius.
...
The \toimprove{novelty} of our result is a deterministic efficient algorithm for computation of the basis for any integer $m$. We \toimprove{exploit} the structure of the group $(\ZZ/m\ZZ)^*$ in a \toimprove{new} way and compute lattice basis through the beauty of duality. {\bf LEO : It is more that we have to {\em deal} with a more complicated structure, than that we {\em exploit} those complication. Also, keep words such as "new" and "novel" for more significant contributions. Smart researcher doing good work do not need to overstate their contributions.}
...
\[
    \LL = \{(x_{1}, \dots, x_{n}) \in \ZZ^{n} |  \forall 1 \leq j \leq k: \sum_{i=1}^{n}x_{i}\log_{\beta_{j}}p_{i}\equiv 0 \pmod{\phi(q_{j}^{e_{j}})}\}
\]
...
\[
\label{parity check}
    \LL_{j} \coloneqq \{(x_{1}, \dots, x_{n}) \in \ZZ^{n} | \sum_{i=1}^{n}x_{i}\log_{\beta_{j}}p_{i}\equiv 0 \pmod{\phi(q_{j}^{e_{j}})}\}
\]
...
We have discussed this representation \toimprove{a lot} in the previous section \ref{definition section}.
\[
    \LL_{j} \coloneqq \{(x_{1}, \dots, x_{n}) \in \ZZ^{n} | \sum_{i=1}^{n}x_{i}\log_{\beta_{j}}p_{i}\equiv 0 \pmod{\phi(q_{j}^{e_{j}})}\}
\]
...
\begin{lemma}
Assume the primes $p_{1}, \dots, p_{n}$ are the same for both lattices. If $a \cdot m \sim b \cdot m' \sim n$, when $n \rightarrow \infty$ Then
\[
