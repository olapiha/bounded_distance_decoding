...
Our work focuses on two families of latices for which BDD problem has an efficient solution and error is close to the Minkowski bound. The first one is a generalization of a lattice discussed in \cite{[DP19]} and used in Chor-Rivest cryptosystem \cite{[CR88]}. The second one was used in \cite{[LLXY17]} for construction of a trapdoor function and has a similar decoding algorithm.

In this work we generalize the lattice family from \cite{[DP19]} suggest an efficient way to construct it. The decoding algorithm of the same article extends to our case directly. The second lattice family was discussed in \cite{[LLXY17]} we extend decoding algorithm devised for them to handle more types of error. We also suggest security improvements for the encryption scheme from \cite{[LLXY17]} using our decoding algorithm and perform cryptanalysis of the mentioned to improve their parameter selection.

\textbf{Discrete logarithm lattices.}
We will discuss many lattice of similar flavour. They are defined as a kernel of a morfism from $\ZZ^{n}$ to another group. Let us give you a short definition of the lattice given in \cite{[DP19]} which is a basis for our first generalization.
...
The kernel of $\psi$ is a subgroup of $\ZZ^{n}$ so it is a lattice. It is easy to see that the image will indeed be an element of $(\ZZ/m\ZZ)^*$ due to our parameter selection.

The basis of this lattice is simple to compute it only requires computation of n discrete logarithms with respect to a smooth modulus \cite{[DP19]}. The generalization we give will require more work in this regard.
...
\textbf{Implemetntation.}
Sagemath implementation of every algorithm for basis computation and message decoding discussed in the document is avaliable on github via link: TBA.
...
In the section \ref{subsec:def_integers} we were able to represent $\LL$ as an intesection of $\LL_{1}, \dots, \LL_{k}$ for which we have parity check representations:
\[
    \LL_{j} \coloneqq \{(x_{1}, \dots, x_{n}) \in \ZZ^{n} | \sum_{i=1}^{n}x_{i}\log_{\beta_{j}}p_{i}\equiv 0 \pmod{\varphi(q_{j}^{e_{j}})}\}
\]
...
The fuction we are considering with fixed parameters $c, b$ tend to the same value as $\frac{bc \cdot n}{\log(n)n^{cb/\log(n)}}$. To simplify the analysis we will find the best parameters for the latter.
