...
\title{Comparing Lattice Families for efficient Bounded Distance Decoding near Minkowski’s Bound}
...
The basis of this lattice is simple to compute it only requires computation of $n$ discrete logarithms in $(\ZZ/m\ZZ)^*$ it can be done because multiplicative group of a field is cylic. Discrete logarithms can calculated in polynomial time as $m$ is a smooth number. More on that you can find in \cite{[DP19]}. The generalization we give will require more work in this regard.
...
The decoding algorithm that we discussed in the introduction \ref{sec:intro} works.

The decoding radius for the general continous error:
\[
    r_{\mathbb{Z}} = \frac{\ln(m/2)}{4 \cdot (\varphi(m))^{1/n} \cdot \ln(B)}
\]

We still haven't configured parameters $k, q_{j}, p_{i}$. $q_{j}, p_{i}$ must be pairwise different and as small as possible, so we need $n+k$ distinct prime numbers. We choose $p_{i}$ to be equal to first $n$ prime numbers and $q_{j}$ equal to the next $k$ smallest primes the we haven't used yet.

The ratio $\frac{ln(m/2)}{\varphi(m)}$ decreases as $m$ tends to infinity, so to make use of the difference in the ratio in the generalizartion, we would like $m$ to have the same size as in the original construction. We would like to have $e_i \sim \frac{\ln(3) \cdot n}{\ln(q) \cdot k}$, but it has to be an integer, so we round this value. Then we have:
\[
...
Let us compare initial construction, its generalization and theoretical upperbound with respect to the decoding radius.

\begin{figure}
  \includegraphics[width=\linewidth]{generalization_integers.png}
  \caption{Comparison of the nomalised decoding radius for initial construction(in green) and the generalization(in yellow). Here we took k = 20. }
  \label{fig:gen_int}
\end{figure}
