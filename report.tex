...
We still haven't configured parameters $k, q_{j}, p_{i}$. $q_{j}, p_{i}$ must be pairwise different and as small as possible, so we need $n+k$ distinct prime numbers. We choose $q_{j}$ to be equal to first $k$ odd prime numbers and $p_{i}$ equal to $2$ and next $n-1$ smallest primes the we haven't used yet.

ToDo: Add that for every $e_i$ we choose $n$.
...
\[
    r_{\mathbb{Z}} = \frac{\ln(m/2)}{4 \cdot (q^{n-1}(q-1))^{1/n} \cdot \ln(B)}
\]
