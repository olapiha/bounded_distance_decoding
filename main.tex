...
\section{Lattice construction for product of fields}
...
\begin{equation}
    \big(\FF_{q}[x]/c(x)\big)^{*} \sim \prod_{i=1}^{k}\FF_{q^{d}}^*
\end{equation}
...
\end{equation}
Or going even further suppose we know $\beta$ a generator of  $\FF_{q^{d}}^*$. Then using the isomorphism between cyclic group $\FF_{q^{d}}^*$ of order $q^{d} - 1$ and group of exponents we get another representation:
\begin{equation}
    \LL = \{(u_{1}, ..., u_{n}) \in \zz^{n} | \sum_{i=1}^{n}u_{i}log_{\beta}(x - \alpha_{i}) \equiv 0 \pmod{\phi(q^{d}-1)}\}
\end{equation}
